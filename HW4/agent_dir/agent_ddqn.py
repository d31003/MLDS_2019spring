...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_ddqn:
            ...
            num = 10000
            self.q_net = load_model('./dqn_model/ddqn_Q_{}.h5'.format(num))
            self.fixed_net = load_model('./dqn_model/ddqn_Fixed_{}.h5'.format(num))
    ...
    def run(self, last_ob, action, reward, terminal, ob):
        ...
        if terminal:
            ...
            plt.xlabel('Episode')
            plt.savefig('./png/ddqn_30_reward.png')
            plt.clf()

            # save model
            if self.episode % 1000 == 0 and self.episode > 0:
                self.q_net.save('./dqn_model/ddqn_Q_{}.h5'.format(self.episode))
                self.fixed_net.save('./dqn_model/ddqn_Fixed_{}.h5'.format(self.episode))

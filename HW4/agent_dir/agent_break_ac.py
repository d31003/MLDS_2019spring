...
class Agent_AC(Agent):
    def __init__(self, env, args):
        ...
        '''
        self.input_frame = Input(shape=(84,84,4))
        self.conv1 = Conv2D(32, (8, 8), strides=(4, 4), activation='relu')(self.input_frame)
        self.conv2 = Conv2D(64, (4, 4), strides=(2, 2), activation='relu')(self.conv1)
        self.conv3 = Conv2D(64, (3, 3), strides=(1, 1), activation='relu')(self.conv2)
        #self.conv1 = Conv2D(32, (3, 3), strides=(1, 1), activation='relu')(self.input_frame)
        self.flat_feature = Flatten()(self.conv2)
        # self.hidden_feature = Dense(512)(self.flat_feature)
        # self.lrelu_feature = LeakyReLU()(self.hidden_feature)
        '''
    ...
    def build_actor(self):
        # hidden_feature1 = Dense(10)(self.flat_feature)
        # #hidden_feature2 = Dense(10)(hidden_feature1)
        # lrelu_feature = LeakyReLU()(hidden_feature1)
        # action = Dense(3, kernel_initializer='RandomNormal', activation='softmax')(lrelu_feature)
        # model = Model(inputs=[self.input_frame], outputs=[action])
        ...
        model.add(Dense(3, kernel_initializer='RandomNormal', activation='softmax'))
        model.compile(loss='categorical_crossentropy', optimizer=self.actor_optimizer, metrics=['accuracy'])
        return model
    ...
    def build_critic(self):
        ...
        # #hidden_feature2 = Dense(10)(hidden_feature1)
        # lrelu_feature = LeakyReLU()(hidden_feature1)
        # value = Dense(1, kernel_initializer='RandomNormal', activation='tanh')(lrelu_feature)
        # model = Model(inputs=[self.input_frame], outputs=[value])
    ...
    def init_game_setting(self):
        ...
        # YOUR CODE HERE #
        ##################
        self.pre_screen = np.zeros((1,84,84,4))
    ...
    def train(self):
        ...
        for i in range(episode):
            ...
            advantage = np.zeros((len(input_ob), 3))
            target = []
            for i in range(len(value) - 1):
                advantage[i][action_list[i]] = reward_array[i] + 0.99 * value[i+1] - value[i]
                target.append(reward_array[i] + 0.99 * value[i+1])
            
            advantage[i+1][action_list[i+1]] = reward_array[i+1] - value[i+1]
            target.append(reward_array[i+1])
            ...
            plt.savefig('./png/break_ac_30_1_reward.png')
            plt.clf()

            if (i + 1) % 200 == 0:
                self.actor.save('./ac_model/break_ac_model_1_{}.h5'.format(i+1))
            # if (i + 1) % 10 == 0:
    ...
    def make_action(self, observation, test=True):
        ...
        #print(act)
        policy = self.actor.predict(ob, batch_size=1).flatten()
        x = np.random.choice(3, 1, p=policy)[0]
        #self.pre_screen = ob
        # print(x)
        # input()
        return x + 1

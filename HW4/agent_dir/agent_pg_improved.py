...
class Agent_PG(Agent):
    ...
    def new_model(self):
        ...
        l1 = Dense(128, input_dim=6400, kernel_initializer='glorot_uniform', activation='relu')(state_input)
        out_actions = Dense(1, kernel_initializer='RandomNormal', activation='sigmoid')(l1)
    ...
    def train(self):
        ...
        for i in range(episode):
            ...
            #advantage.reshape((1,))
            result_new = self.new.fit([input_ob, advantage, pred], [action_array], batch_size=300,
                epochs=1, verbose=1)#, sample_weight=reward_array
            
            result_old = self.old.fit(input_ob, reward_array, batch_size=300,
            ...
            plt.savefig('./png/pg_improved_30_h_reward.png')
            plt.clf()

            if (i + 1) % 200 == 0:
                self.new.save('./pg_model/pg_improved_model_h_{}.h5'.format(i+1))
            # if (i + 1) % 10 == 0:
    ...
    def make_action(self, observation, test=True):
        ...
        act = self.new.predict([ob - self.pre_screen, dummy_value, dummy_action])
        #print(act)
        self.pre_screen = ob

...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_dddqn:
            ...
            num = 17000
            self.q_net = load_model('./dqn_model/dddqn_Q_{}.h5'.format(num))
            self.fixed_net = load_model('./dqn_model/dddqn_Fixed_{}.h5'.format(num))
    ...
    def make_model(self):
        ...
        if self.dueling:
            ...
            print('Dueling Initialized!')
            hidden_feature_2 = Dense(512,activation='relu')(flat_feature)
            state_value_prediction = Dense(1)(hidden_feature_2)
            q_value_prediction = Lambda(lambda x: x[0]-K.mean(x[0])+x[1], output_shape = (4,))([q_value_prediction, state_value_prediction])
    ...
    def run(self, last_ob, action, reward, terminal, ob):
        ...
        if terminal:
            ...
            plt.xlabel('Episode')
            plt.savefig('./png/dddqn_30_reward.png')
            plt.clf()

            # save model
            if self.episode % 1000 == 0 and self.episode > 0:
                self.q_net.save('./dqn_model/dddqn_Q_{}.h5'.format(self.episode))
                self.fixed_net.save('./dqn_model/dddqn_Fixed_{}.h5'.format(self.episode))
    ...
    def train_network(self):
        ...
        if not self.ddqn:
            ...
            pred_action = np.argmax( self.q_net.predict([list2np(next_state_batch),self.dummy_batch])[0], axis=-1)
            for i in range(self.batch_size):
                y_batch.append(reward_batch[i] + (1 - terminal_batch[i]) * self.gamma * target_q_values_batch[i][pred_action[i]] )

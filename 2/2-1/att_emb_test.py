...
#########
model=load_model('model/att_sample_{}_unk_epoch{}_dim{}_16.h5'.format(sample_num,EPOCH,latent_dim))
##########

for T in range(17,sample_num):
    print('round', T)

    #if T==0:
    #    model=load_model('model/att_sample_{}_unk_epoch{}_dim{}_{}.h5'.format(sample_num,EPOCH,latent_dim))
    ...
    tem = train_label[:,:34,:]
    gt=np.concatenate((out,tem),axis=1)
    del tem
    model.fit([train_data,gt],train_label1,epochs=EPOCH, batch_size=10, verbose=1, shuffle=True)
    model.save('model/att_sample_{}_unk_epoch{}_dim{}_{}.h5'.format(sample_num,EPOCH,latent_dim,T))
...
model.save('model/att_sample_{}_unk_epoch{}_dim{}.h5'.format(sample_num,EPOCH,latent_dim))
'''
...
for T in range(20,24):
    model=load_model('model/att_sample_{}_unk_d{}_epoch{}_dim{}_{}.h5'.format(sample_num,dict_min,EPOCH,latent_dim,T))
    ...
    f=open('model/att_sample_{}_unk_d{}_epoch{}_dim{}_{}.txt'.format(sample_num,dict_min,EPOCH,latent_dim,T),'w')
    f.write(answer)
